...
rpmuncompress -x 'v1.13.0.tar.gz'
STATUS=$?
if [ $STATUS -ne 0 ]; then
  exit $STATUS
fi
...
{ rpmuncompress libkrun-remove-unused-deps.diff || echo patch_fail ; } | patch --no-backup-if-mismatch -f -p1  --fuzz=0

echo "Patch #1 (libkrun-remove-sev-deps.diff):"
{ rpmuncompress libkrun-remove-sev-deps.diff || echo patch_fail ; } | patch --no-backup-if-mismatch -f -p1  --fuzz=0
