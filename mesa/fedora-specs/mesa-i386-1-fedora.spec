...
Requires:       mesa-dri-drivers(x86-32) = 26.1.5-1
...
Requires:       (mesa-libGL(x86-32) = 26.1.5-1 if mesa-libGL(x86-32))
Requires:       libglvnd-devel(x86-32) >= 1:1.3.2
Provides:       libGL-devel = 26.1.5-1
Provides:       libGL-devel(x86-32) = 26.1.5-1
...
Requires:       mesa-libgbm(x86-32) = 26.1.5-1
Requires:       mesa-dri-drivers(x86-32) = 26.1.5-1
...
Requires:       (mesa-libEGL(x86-32) = 26.1.5-1 if mesa-libEGL(x86-32))
Requires:       libglvnd-devel(x86-32) >= 1:1.3.2
Requires:       mesa-khr-devel(x86-32)
Provides:       libEGL-devel = 26.1.5-1
Provides:       libEGL-devel(x86-32) = 26.1.5-1
...
Provides:       libgbm(x86-32) = 26.1.5-1
Recommends:     mesa-dri-drivers(x86-32) = 26.1.5-1

Requires:       (mesa-dri-drivers(x86-32) = 26.1.5-1 if mesa-dri-drivers(x86-32))
...
Provides:       libgbm-devel(x86-32) = 26.1.5-1
...
Requires:       mesa-libOpenCL(x86-32) = 26.1.5-1
...
cd './'
rm -rf 'mesa-26.1.5'
rpmuncompress -x 'mesa-26.1.5.tar.xz'
STATUS=$?
if [ $STATUS -ne 0 ]; then
  exit $STATUS
fi
...
%meson \
  ...
  -Dgallium-drivers=llvmpipe,virgl,nouveau,r300,crocus,i915,iris,svga,radeonsi,r600,asahi,freedreno,etnaviv,tegra,vc4,v3d,lima,panfrost,zink,d3d12 \
  -Dgallium-va=enabled \
...
ln -s libGLX_mesa.so.0 fakeinstall/usr/lib32/libGLX_system.so.0
...
/usr/lib32/dri/d3d12_drv_video.so
/usr/lib32/dri/virtio_gpu_drv_video.so

%files dxil-devel
/usr/bin/spirv2dxil
/usr/lib32/libspirv_to_dxil.a
/usr/lib32/libspirv_to_dxil.so
