  ...
  Requires:       mesa-dri-drivers(x86-64) = 26.1.5-1
  ...
  Requires:       (mesa-libGL(x86-64) = 26.1.5-1 if mesa-libGL(x86-64))
  Requires:       libglvnd-devel(x86-64) >= 1:1.3.2
  Provides:       libGL-devel = 26.1.5-1
  Provides:       libGL-devel(x86-64) = 26.1.5-1
  ...
  Requires:       mesa-libgbm(x86-64) = 26.1.5-1
  Requires:       mesa-dri-drivers(x86-64) = 26.1.5-1
  ...
  Requires:       (mesa-libEGL(x86-64) = 26.1.5-1 if mesa-libEGL(x86-64))
  Requires:       libglvnd-devel(x86-64) >= 1:1.3.2
  Requires:       mesa-khr-devel(x86-64)
  Provides:       libEGL-devel = 26.1.5-1
  Provides:       libEGL-devel(x86-64) = 26.1.5-1
  ...
  Provides:       libgbm(x86-64) = 26.1.5-1
  Recommends:     mesa-dri-drivers(x86-64) = 26.1.5-1

  Requires:       (mesa-dri-drivers(x86-64) = 26.1.5-1 if mesa-dri-drivers(x86-64))
  ...
  Provides:       libgbm-devel(x86-64) = 26.1.5-1
  ...
  Requires:       mesa-libOpenCL(x86-64) = 26.1.5-1
...
build() {
  ...
  %meson \
    ...
    -Dgallium-drivers=llvmpipe,virgl,nouveau,r300,crocus,i915,iris,svga,radeonsi,r600,asahi,freedreno,etnaviv,tegra,vc4,v3d,lima,panfrost,zink,d3d12,ethosu,rocket \
    -Dgallium-va=enabled \
...
package() {
  ...
  _install fakeinstall/usr/$LIBDIR/dri/d3d12_drv_video.so
  _install fakeinstall/usr/$LIBDIR/dri/virtio_gpu_drv_video.so

  # dxil-devel
  _install fakeinstall/usr/bin/spirv2dxil
  _install fakeinstall/usr/$LIBDIR/libspirv_to_dxil.a
  _install fakeinstall/usr/$LIBDIR/libspirv_to_dxil.so
