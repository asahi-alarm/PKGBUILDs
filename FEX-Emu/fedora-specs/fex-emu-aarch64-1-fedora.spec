...
cd './'
rm -rf 'FEX-FEX-2412'
rpmuncompress -x 'FEX-FEX-2412.tar.gz'
STATUS=$?
if [ $STATUS -ne 0 ]; then
  exit $STATUS
fi
...
rpmuncompress 0001-Thunks-gen-Add-support-for-compiling-against-clang-1.patch | 
patch -p1 -s --fuzz=0 --no-backup-if-mismatch -f

rpmuncompress 0001-Library-Forwarding-Allow-reading-standard-library-he.patch | 
patch -p1 -s --fuzz=0 --no-backup-if-mismatch -f
