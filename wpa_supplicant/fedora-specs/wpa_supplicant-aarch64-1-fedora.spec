...
rpmuncompress -x 'wpa_supplicant-2.11.tar.gz'
STATUS=$?
if [ $STATUS -ne 0 ]; then
  exit $STATUS
fi
...
rpmuncompress wpa_supplicant-defconfig-keep-options-we-ve-traditionally-used-enab.patch | 
patch -p1 -s --fuzz=0 --no-backup-if-mismatch -f

rpmuncompress wpa_supplicant-assoc-timeout.patch | 
patch -p1 -s --fuzz=0 --no-backup-if-mismatch -f

rpmuncompress wpa_supplicant-flush-debug-output.patch | 
patch -p1 -s --fuzz=0 --no-backup-if-mismatch -f

rpmuncompress wpa_supplicant-quiet-scan-results-message.patch | 
patch -p1 -s --fuzz=0 --no-backup-if-mismatch -f

rpmuncompress wpa_supplicant-gui-qt4.patch | 
patch -p1 -s --fuzz=0 --no-backup-if-mismatch -f

rpmuncompress wpa_supplicant-defconfig-keep-CONFIG_WEP-enabled.patch | 
patch -p1 -s --fuzz=0 --no-backup-if-mismatch -f

rpmuncompress wpa_supplicant-defconfig-enable-WPA-EAP-SUITE-B-192-ciphers.patch | 
patch -p1 -s --fuzz=0 --no-backup-if-mismatch -f

rpmuncompress wpa_supplicant-defconfig-enable-OCV-support.patch | 
patch -p1 -s --fuzz=0 --no-backup-if-mismatch -f

rpmuncompress wpa_supplicant-allow-legacy-renegotiation.patch | 
patch -p1 -s --fuzz=0 --no-backup-if-mismatch -f

rpmuncompress wpa_supplicant-Revert-Mark-authorization-completed-on-driver-indica.patch | 
patch -p1 -s --fuzz=0 --no-backup-if-mismatch -f

rpmuncompress wpa_supplicant-Send-signal-change-as-debug-msg.patch | 
patch -p1 -s --fuzz=0 --no-backup-if-mismatch -f

rpmuncompress wpa_supplicant-OpenSSL-Use-pkcs11-provider-when-OPENSSL_NO_ENGINE-i.patch | 
patch -p1 -s --fuzz=0 --no-backup-if-mismatch -f

rpmuncompress wpa_supplicant-OpenSSL-Support-PEM-encoded-chain-from-ca_cert-blob.patch | 
patch -p1 -s --fuzz=0 --no-backup-if-mismatch -f
