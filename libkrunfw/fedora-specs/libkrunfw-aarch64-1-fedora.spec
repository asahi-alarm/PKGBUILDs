...
Source0:        https://github.com/containers/libkrunfw/archive/refs/tags/v5.5.0.tar.gz

Source1:        https://www.kernel.org/pub/linux/kernel/v6.x/linux-6.12.91.tar.xz
...
Requires: libkrunfw(aarch-64) = 5.5.0-1
...
cd './'
rm -rf 'libkrunfw-5.5.0'
rpmuncompress -x 'v5.5.0.tar.gz'
STATUS=$?
if [ $STATUS -ne 0 ]; then
  exit $STATUS
fi
...
GIT_COMMITTER_DATE=@${SOURCE_DATE_EPOCH:-${RPM_BUILD_TIME:?}} GIT_AUTHOR_DATE=@${SOURCE_DATE_EPOCH:-${RPM_BUILD_TIME:?}}\
	...
	--author "rpm-build <rpm-build>" -m "libkrunfw-5.5.0 base"
/usr/bin/git checkout --track -b rpm-build

mkdir tarballs
cp linux-6.12.91.tar.xz tarballs/
